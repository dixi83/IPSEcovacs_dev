    ...
    public function XMPPcommand($robotSerialNr,$command) { // just send message, <iq type="set"> will not get any responce from ecovacs servers
        ...
        switch ($command) {
            ...
            case self::SET_PLAYSOUND:
                $GetMessage = '<query xmlns="com:ctl"><ctl id="12351409" sid="1" td="PlaySound" /></query>';
                $ExpectReply = false;
                break;
        ...
        if (!(count($XmppReply) > 1)) { // if it is correctly answered there should be 2 or more messages
