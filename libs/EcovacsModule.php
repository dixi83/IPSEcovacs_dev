...
class EcovacsXMPP extends IPSModule {
    ...
    public function XMPPgetCommand($robotSerialNr,$command) { // just send message, <iq type="set"> will not get any responce from ecovacs servers
        ...
        $message->setMessage($GetMessage)
            ->setTo($set['vacAddr'])
            ->setFrom($set['username'].'@'.$set['domain'].'/12345678')//.$set['resource']) //self.user + '@' + self.domain + '/' + self.resource
        ...
        while(true) { // wait for messages
        	$messages = $client->getMessages(true); //$client->getConnection()->receive(); //$client->getMessages(true);    
        	if((count($messages) > 0) or ((time()-$startTime)>5)) {
        		break;
        	}
        }
